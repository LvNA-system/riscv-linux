...
static int axienet_probe(struct platform_device *pdev)
{
  ...
  */
  dmares = platform_get_resource_byname(pdev, IORESOURCE_MEM, "dma");
	if (!dmares) {
  ...
  irqres = platform_get_resource_byname(pdev, IORESOURCE_IRQ, "rx");
  lp->rx_irq = (irqres != NULL ? irqres->start : 0);
  irqres = platform_get_resource_byname(pdev, IORESOURCE_IRQ, "tx");
  lp->tx_irq = (irqres != NULL ? irqres->start : 0);
	//lp->rx_irq = irq_of_parse_and_map(np, 1);
	//lp->tx_irq = irq_of_parse_and_map(np, 0);
	printk(KERN_ERR "Rx irq: %d, Tx irq: %d\n", lp->rx_irq, lp->tx_irq);
	//of_node_put(np);
